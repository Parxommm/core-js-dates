...
function getNextFriday(date) {
  ...
  const dayToFriday = 4 - currentDay;
  let dayToNextFriday;

  if (dayToFriday === 0) dayToNextFriday = 7;

  if (dayToFriday > 0) dayToNextFriday = dayToFriday;

  if (dayToFriday < 0) dayToNextFriday = dayToFriday + 7;

  return new Date(date.setDate(date.getDate() + dayToNextFriday + 1));
}
...
function formatDate(date) {
  ...
  const timeWithTimezone = new Date(time.getTime() + timezone * 60 * 1000);
  return timeWithTimezone.toLocaleDateString('en-US', {
    hour: 'numeric',
    minute: '2-digit',
    second: '2-digit',
  });
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= countDays; i += 1) {
    const weekDay = new Date(year, month - 1, i).toLocaleDateString('en-US', {
      weekday: 'short',
    });
...
function getWeekNumberByDate(date) {
  ...
  const daysCount = Math.floor((date - firstJan) / (24 * 60 * 60 * 1000));
  ...
  if (firstJanDay === 0) {
    ...
    firstWeekDay = firstJanDay;
  }

  return Math.ceil((daysCount + firstWeekDay) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (!friday13) {
    const newDate = new Date(date.getFullYear(), month, 13);
    if (newDate.getDay() === 5) {
      friday13 = newDate;
      break;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startArr = period.start.split('-').map((el) => Number(el));
  const endArr = period.end.split('-').map((el) => Number(el));
  ...
  while (start <= end) {
    ...
    for (let i = countOffDays; i > 0; i -= 1) {
      start.setDate(start.getDate() + 1);
    }
  }
